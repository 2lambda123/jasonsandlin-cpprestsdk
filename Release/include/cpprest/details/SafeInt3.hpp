...
contributors' name, logo, or trademarks. (B) If you bring a patent claim against
any contributor over patents that you claim are infringed by the software, your
patent license from such contributor to the software ends automatically. (C) If
you distribute any portion of the software, you must retain all copyright,
patent, trademark, and attribution notices that are present in the software. (D)
...
license. (E) The software is licensed "as-is." You bear the risk of using it.
...
SafeInt supports several compile-time options that can change the behavior of
the class.

Compiler options:
SAFEINT_WARN_64BIT_PORTABILITY     - this re-enables various warnings that
happen when /Wp64 is used. Enabling this option is not recommended.
NEEDS_INT_DEFINED                  - if your compiler does not support __int8,
__int16, __int32 and __int64, you can enable this. SAFEINT_ASSERT_ON_EXCEPTION
- it is often easier to stop on an assert and figure out a problem than to try
and figure out how you landed in the catch block. SafeIntDefaultExceptionHandler
- if you'd like to replace the exception handlers SafeInt provides, define your
replacement and define this. Note - two built in (Windows-specific) options
exist:
                                     - SAFEINT_FAILFAST - bypasses all exception
handlers, exits the app with an exception
                                     - SAFEINT_RAISE_EXCEPTION - throws Win32
exceptions, which can be caught SAFEINT_DISALLOW_UNSIGNED_NEGATION - Invoking
the unary negation operator creates warnings, but if you'd like it to completely
fail to compile, define this. ANSI_CONVERSIONS                   - This changes
the class to use default comparison behavior, which may be unsafe. Enabling this
                                     option is not recommended.
SAFEINT_DISABLE_BINARY_ASSERT      - binary AND, OR or XOR operations on mixed
size types can produce unexpected results. If you do this, the default is to
assert. Set this if you prefer not to assert under these conditions.
SIZE_T_CAST_NEEDED                 - some compilers complain if there is not a
cast to size_t, others complain if there is one. This lets you not have your
compiler complain. SAFEINT_DISABLE_SHIFT_ASSERT       - Set this option if you
don't want to assert when shifting more bits than the type has. Enabling this
option is not recommended.
...
*  acceptable.
*
*  One key operator that is missing is an implicit cast to type T. The reason
for
...
*  integrity of the calculations is more important than performance. See
Performance
...
*  unsigned integer types - certain types of problems cannot occur, and this
class
...
*  You can replace the exception class with any exception class you like. This
is accomplished by:
...
*
*  Note that you don't have to throw C++ exceptions, you can throw Win32
exceptions, or do
...
*  performance in unoptimized code. In tests of optimized code vs. correct
inline checks
*  in native code, this class has been found to take approximately 8% more CPU
time (this varies),
*  most of which is due to exception handling. Solutions:
*
*  1) Compile optimized code - /Ox is best, /O2 also performs well.
Interestingly, /O1
*     (optimize for size) does not work as well.
*  2) If that 8% hit is really a serious problem, walk through the code and
inline the
*     exact same checks as the class uses.
*  3) Some operations are more difficult than others - avoid using signed
integers, and if
*     possible keep them all the same size. 64-bit integers are also expensive.
Mixing
*     different integer sizes and types may prove expensive. Be aware that
literals are
*     actually ints. For best performance, cast literals to the type desired.
*
*
*  Performance update
*  The current version of SafeInt uses template specialization to force the
compiler to invoke only the
*  operator implementation needed for any given pair of types. This will
dramatically improve the perf
*  of debug builds.
*
*  3.0 update - not only have we maintained the specialization, there were some
cases that were overly complex,
*  and using some additional cases (e.g. signed __int64 and unsigned __int64)
resulted in some simplification.
*  Additionally, there was a lot of work done to better optimize the 64-bit
multiplication.
*
*  Binary Operators
*
*  All of the binary operators have certain assumptions built into the class
design.
...
*  U op SafeInt< T, E >
*
*  The SafeInt< T, E > op SafeInt< U, E > variant is explicitly not supported,
and if you try to do
*  this the compiler with throw the following error:
*
*  error C2593: 'operator *' is ambiguous
*
*  This is because the arithmetic operators are required to return a SafeInt of
some type.
*  The compiler cannot know whether you'd prefer to get a type T or a type U
returned. If
*  you need to do this, you need to extract the value contained within one of
the two using
...
*  Because each of these operators return type bool, mixing SafeInts of
differing types is
*  allowed.
*
*  Shift Operators
*  Shift operators always return the type on the left hand side of the operator.
Mixed type
*  operations are allowed because the return type is always known.
*
*  Boolean Operators
*  Like comparison operators, these overloads always return type bool, and
mixed-type SafeInts
*  are allowed. Additionally, specific overloads exist for type bool on both
sides of the
*  operator.
*
*  Binary Operators
*  Mixed-type operations are discouraged, however some provision has been made
in order to
*  enable things like:
*
*  SafeInt<char> c = 2;
...
*  In the case of binary operations on integers smaller than 32-bit, or of mixed
type, corner
*  cases do exist where you could get unexpected results. In any case where
SafeInt returns a different
*  result than the underlying operator, it will call assert(). You should
examine your code and cast things
...
and later.
*  As of this writing, I believe it will also work for VC 7.1, but not for
VC 7.0 or below.
*  If you need a version that will work with lower level compilers, try
version 1.0.7. None
*  of them work with Visual C++ 6, and gcc didn't work very well, either, though
this hasn't
*  been tried recently.
*
*  It is strongly recommended that any code doing integer manipulation be
compiled at /W4
*  - there are a number of warnings which pertain to integer manipulation
enabled that are
*  not enabled at /W3 (default for VC++)
*
*  Perf note - postfix operators are slightly more costly than prefix operators.
*  Unless you're actually assigning it to something, ++SafeInt is less expensive
than SafeInt++
*
*  The comparison operator behavior in this class varies from the ANSI
definition, which is
...
*
*  The problem here is that c gets cast to an int, now has a value of
0xffffffff, and then gets
*  cast again to an unsigned int, losing the true value. This behavior is
despite the fact that
*  an __int64 exists, and the following code will yield a different (and
intuitively correct)
*  answer:
*
...
*
*  Note that combinations with smaller integers won't display the problem - if
you
...
*
*  Jan 16, 2010   64-bit gcc has long == __int64, which means that many of the
existing 64-bit
...
* Version 3.0 changes:
*
* 1) The exception type thrown is now replacable, and you can throw your own
exception types. This should help
*    those using well-developed exception classes.
* 2) The 64-bit multiplication code has had a lot of perf work done, and should
be faster than 2.0.
* 3) There is now limited floating point support. You can initialize a SafeInt
with a floating point type,
*    and you can cast it out (or assign) to a float as well.
* 4) There is now an Align method. I noticed people use this a lot, and rarely
check errors, so now you have one.
*
* Another major improvement is the addition of external functions - if you just
want to check an operation, this can now happen:
* All of the following can be invoked without dealing with creating a class, or
managing exceptions. This is especially handy
* for 64-bit porting, since SafeCast compiles away for a 32-bit cast from size_t
to unsigned long, but checks it for 64-bit.
*
 ...
 * If for some reason you have to translate between the two, cast one of them
 * back to its native type.
 *
 * To use your own exception class with SafeInt, first create your exception
 * class, which may look something like the SafeIntException class below. The
 * second step is to create a template specialization that implements
 * SafeIntOnOverflow and SafeIntOnDivZero. For example:
 *
 ...
 * };
 *
 * typedef SafeIntExceptionHandler < YourExceptionClass >
 * YourSafeIntExceptionHandler You'd then declare your SafeInt objects like
 * this: SafeInt< int, YourSafeIntExceptionHandler >
 *
 * Unfortunately, there is no such thing as partial template specialization in
 * typedef statements, so you have three options if you find this cumbersome:
 *
 ...
 * MySafeInt< int > i;
 *
 * You'd lose handy things like initialization - it would have to be initialized
 * as:
 *
 * i.si = 0;
 *
 ...
 *
 * and so on. The second approach is probably more usable, and will just drop
 * into code better, which is the original intent of the SafeInt class.
 *
 * 3) If you're going to consistently use a different class to handle your
 * exceptions, you can override the default typedef like so:
 *
 *    #define SafeIntDefaultExceptionHandler YourSafeIntExceptionHandler
 *
...
// exceptions, which makes everything no throw.
...
// handlers:
// SafeIntErrorPolicy_SafeIntException - C++ exception, default if not otherwise
// defined SafeIntErrorPolicy_InvalidParameter - Calls fail fast
// (Windows-specific), bypasses any exception handlers,
...
typedef SafeIntInternal::SafeIntExceptionHandler<SafeIntException>
    CPlusPlusExceptionHandler;
typedef SafeIntInternal::SafeInt_InvalidParameter
    InvalidParameterExceptionHandler;

// This exception handler is no longer recommended, but is left here in order
// not to break existing users
#if defined _WINDOWS_
...
// Some users might mix exception handlers, which is not advised, but is
// supported
...
// Turns out we can fool the compiler into not seeing compile-time constants
// with a simple template specialization
...
// from compiling in the case of an enum, which is the point of the
// specialization that follows.
...
template <typename T> class NumericType {
public:
  // We have some unknown type, which could be an enum. For parity with the code
  // that uses <type_traits>, We can try a static_cast<int> - it if compiles,
  // then it might be an enum, and should work. If it is something else that
  // just happens to have a constructor that takes an int, and a casting
  // operator, then it is possible something will go wrong, and for best
  // results, cast it directly to an int before letting it interact with a
  // SafeInt
...
public:
  ...
  // These must be cast to deal with the possibility of a SafeInt being given an
  // enum as an argument
  const static T maxInt =
      static_cast<T>(SafeIntMinMax<isSigned, bitCount>::max);
  const static T minInt =
      static_cast<T>(SafeIntMinMax<isSigned, bitCount>::min);
};
...
public:
  enum {
    ...
    IntZone_UintLT32_UintLT32 = SafeIntCompare<T, U>::isBothUnsigned &&
                                SafeIntCompare<T, U>::isBothLT32Bit,
    IntZone_Uint32_UintLT64 = SafeIntCompare<T, U>::isBothUnsigned &&
                              IntTraits<T>::is32Bit && IntTraits<U>::isLT64Bit,
    IntZone_UintLT32_Uint32 = SafeIntCompare<T, U>::isBothUnsigned &&
                              IntTraits<T>::isLT32Bit && IntTraits<U>::is32Bit,
    IntZone_Uint64_Uint =
        SafeIntCompare<T, U>::isBothUnsigned && IntTraits<T>::is64Bit,
    IntZone_UintLT64_Uint64 = SafeIntCompare<T, U>::isBothUnsigned &&
                              IntTraits<T>::isLT64Bit && IntTraits<U>::is64Bit,
    // unsigned-signed
    IntZone_UintLT32_IntLT32 = !IntTraits<T>::isSigned &&
                               IntTraits<U>::isSigned &&
                               SafeIntCompare<T, U>::isBothLT32Bit,
    IntZone_Uint32_IntLT64 = IntTraits<T>::isUint32 && IntTraits<U>::isSigned &&
                             IntTraits<U>::isLT64Bit,
    IntZone_UintLT32_Int32 = !IntTraits<T>::isSigned &&
                             IntTraits<T>::isLT32Bit && IntTraits<U>::isInt32,
    IntZone_Uint64_Int = IntTraits<T>::isUint64 && IntTraits<U>::isSigned &&
                         IntTraits<U>::isLT64Bit,
    IntZone_UintLT64_Int64 = !IntTraits<T>::isSigned &&
                             IntTraits<T>::isLT64Bit && IntTraits<U>::isInt64,
    IntZone_Uint64_Int64 = IntTraits<T>::isUint64 && IntTraits<U>::isInt64,
    // signed-signed
    IntZone_IntLT32_IntLT32 = SafeIntCompare<T, U>::isBothSigned &&
                              SafeIntCompare<T, U>::isBothLT32Bit,
    IntZone_Int32_IntLT64 = SafeIntCompare<T, U>::isBothSigned &&
                            IntTraits<T>::is32Bit && IntTraits<U>::isLT64Bit,
    IntZone_IntLT32_Int32 = SafeIntCompare<T, U>::isBothSigned &&
                            IntTraits<T>::isLT32Bit && IntTraits<U>::is32Bit,
    IntZone_Int64_Int64 = SafeIntCompare<T, U>::isBothSigned &&
                          IntTraits<T>::isInt64 && IntTraits<U>::isInt64,
    IntZone_Int64_Int = SafeIntCompare<T, U>::isBothSigned &&
                        IntTraits<T>::is64Bit && IntTraits<U>::isLT64Bit,
    IntZone_IntLT64_Int64 = SafeIntCompare<T, U>::isBothSigned &&
                            IntTraits<T>::isLT64Bit && IntTraits<U>::is64Bit,
    // signed-unsigned
    IntZone_IntLT32_UintLT32 = IntTraits<T>::isSigned &&
                               !IntTraits<U>::isSigned &&
                               SafeIntCompare<T, U>::isBothLT32Bit,
    IntZone_Int32_UintLT32 = IntTraits<T>::isInt32 && !IntTraits<U>::isSigned &&
                             IntTraits<U>::isLT32Bit,
    IntZone_IntLT64_Uint32 = IntTraits<T>::isSigned &&
                             IntTraits<T>::isLT64Bit && IntTraits<U>::isUint32,
    IntZone_Int64_UintLT64 = IntTraits<T>::isInt64 && !IntTraits<U>::isSigned &&
                             IntTraits<U>::isLT64Bit,
    IntZone_Int_Uint64 = IntTraits<T>::isSigned && IntTraits<U>::isUint64 &&
                         IntTraits<T>::isLT64Bit,
...
public:
  enum {
    method = IntTraits<T>::isLT64Bit && IntTraits<T>::isSigned ? AbsMethodInt
             : IntTraits<T>::isInt64                           ? AbsMethodInt64
                                                               : AbsMethodNoop
  };
};
...
{
public:
  template <typename E> static T NegativeThrow(T t) SAFEINT_CPP_THROW {
    // corner case
...
template <> class SignedNegation<signed __int32> {
public:
...
template <> class SignedNegation<signed __int64> {
public:
...
#if SAFEINT_COMPILER == VISUAL_STUDIO_COMPILER
#pragma warning(push)
// this avoids warnings from the unary '-' operator being applied to unsigned
// numbers
...
public:
  ...
  template <typename E> static void CastThrow(bool b, T &t) SAFEINT_CPP_THROW {
    t = (T)(b ? 1 : 0);
  }
};
  ...
  enum {
    ...
    method = ((SafeIntCompare<T, U>::isLikeSigned) ? ComparisonMethod_Ok
              ...
              : ((IntTraits<T>::isSigned && sizeof(U) < 8) ||
                 (IntTraits<U>::isSigned && sizeof(T) < 8))
                  ? ComparisonMethod_CastInt64
...
public:
  static SafeIntError Modulus(const T &t, const U &u,
  ...
  template <typename E>
  static void ModulusThrow(const T &t, const U &u,
  ...
  enum {
    ...
    method =
        (IntRegion<T, U>::IntZone_UintLT32_UintLT32
         ...
         : SafeIntCompare<T, U>::isBothUnsigned && IntTraits<T>::isUint64 &&
                 IntTraits<U>::isUint64
...
public:
  ...
  static bool Multiply(const T &t, const U &u, T &ret) SAFEINT_NOTHROW {
    ...
    if (tmp > (__int64)IntTraits<T>::maxInt ||
        tmp < (__int64)IntTraits<T>::minInt)
      return false;
...
inline bool IntrinsicMultiplyInt64(const signed __int64 &a,
  ...
  *pRet = _mul128(a, b, &llHigh);
  ...
  if ((a ^ b) < 0) {
    ...
    // Check for overflow
    if (llHigh == 0 &&
        (unsigned __int64)*pRet <= IntTraits<signed __int64>::maxInt)
      return true;
  }
  return false;
}
...
public:
  static bool RegMultiply(const unsigned __int64 &a, const unsigned __int64 &b,
                          unsigned __int64 *pRet) SAFEINT_NOTHROW {
#if SAFEINT_USE_INTRINSICS
...
public:
  ...
  template <typename E>
  static void RegMultiplyThrow(const unsigned __int64 &a, unsigned __int32 b,
                               unsigned __int64 *pRet) SAFEINT_CPP_THROW {
#if SAFEINT_USE_INTRINSICS
...
public:
  ...
  static bool RegMultiply(const unsigned __int64 &a, signed __int32 b,
                          unsigned __int64 *pRet) SAFEINT_NOTHROW {
    if (b < 0 && a != 0)
      return false;
  ...
  template <typename E>
  static void RegMultiplyThrow(const unsigned __int64 &a, signed __int32 b,
                               unsigned __int64 *pRet) SAFEINT_CPP_THROW {
    if (b < 0 && a != 0)
      E::SafeIntOnOverflow();
    ...
    if (!IntrinsicMultiplyUint64(a, (unsigned __int64)b, pRet))
      E::SafeIntOnOverflow();
#else
    LargeIntRegMultiply<unsigned __int64, unsigned __int32>::
        template RegMultiplyThrow<E>(a, (unsigned __int32)b, pRet);
#endif
  }
};
...
public:
  static bool RegMultiply(const unsigned __int64 &a, signed __int64 b,
                          unsigned __int64 *pRet) SAFEINT_NOTHROW {
    if (b < 0 && a != 0)
      return false;
  ...
  template <typename E>
  static void RegMultiplyThrow(const unsigned __int64 &a, signed __int64 b,
                               unsigned __int64 *pRet) SAFEINT_CPP_THROW {
    if (b < 0 && a != 0)
      E::SafeIntOnOverflow();
    ...
    if (!IntrinsicMultiplyUint64(a, (unsigned __int64)b, pRet))
      E::SafeIntOnOverflow();
#else
    LargeIntRegMultiply<unsigned __int64, unsigned __int64>::
        template RegMultiplyThrow<E>(a, (unsigned __int64)b, pRet);
#endif
  }
};
...
public:
  ...
  static bool RegMultiply(signed __int32 a, const unsigned __int64 &b,
                          signed __int32 *pRet) SAFEINT_NOTHROW {
    ...
    *pRet = 0;
    ...
    if (a < 0) {
      ...
      a = (signed __int32)AbsValueHelper<
          signed __int32, GetAbsMethod<signed __int32>::method>::Abs(a);
      fIsNegative = true;
    }
  ...
  template <typename E>
  static void RegMultiplyThrow(signed __int32 a, const unsigned __int64 &b,
                               signed __int32 *pRet) SAFEINT_CPP_THROW {
    unsigned __int32 bHigh, bLow;
    bool fIsNegative = false;
...
public:
  ...
  static bool RegMultiply(unsigned __int32 a, const unsigned __int64 &b,
                          unsigned __int32 *pRet) SAFEINT_NOTHROW {
    // Consider that a*b can be broken up into:
    // (bHigh * 2^32 + bLow) * a
    // => (bHigh * a * 2^32) + (bLow * a)
  ...
  template <typename E>
  static void RegMultiplyThrow(unsigned __int32 a, const unsigned __int64 &b,
...
public:
  static bool RegMultiply(unsigned __int32 a, const signed __int64 &b,
                          unsigned __int32 *pRet) SAFEINT_NOTHROW {
    if (b < 0 && a != 0)
      return false;
    return LargeIntRegMultiply<unsigned __int32, unsigned __int64>::RegMultiply(
        a, (unsigned __int64)b, pRet);
  }

  template <typename E>
  static void RegMultiplyThrow(unsigned __int32 a, const signed __int64 &b,
                               unsigned __int32 *pRet) SAFEINT_CPP_THROW {
    if (b < 0 && a != 0)
      E::SafeIntOnOverflow();

    LargeIntRegMultiply<unsigned __int32, unsigned __int64>::
        template RegMultiplyThrow<E>(a, (unsigned __int64)b, pRet);
  }
};
...
public:
  static bool RegMultiply(const signed __int64 &a, const signed __int64 &b,
                          signed __int64 *pRet) SAFEINT_NOTHROW {
#if SAFEINT_USE_INTRINSICS
    ...
    if (a1 < 0) {
      aNegative = true;
      a1 = (signed __int64)AbsValueHelper<
          signed __int64, GetAbsMethod<signed __int64>::method>::Abs(a1);
    }

    if (b1 < 0) {
      bNegative = true;
      b1 = (signed __int64)AbsValueHelper<
          signed __int64, GetAbsMethod<signed __int64>::method>::Abs(b1);
    }
  ...
  template <typename E>
  static void RegMultiplyThrow(const signed __int64 &a, const signed __int64 &b,
    ...
    LargeIntRegMultiply<unsigned __int64, unsigned __int64>::
        template RegMultiplyThrow<E>((unsigned __int64)a1, (unsigned __int64)b1,
                                     &tmp);
...
public:
  static bool RegMultiply(const signed __int64 &a, unsigned __int32 b,
                          signed __int64 *pRet) SAFEINT_NOTHROW {
#if SAFEINT_USE_INTRINSICS
  ...
  template <typename E>
  static void RegMultiplyThrow(const signed __int64 &a, unsigned __int32 b,
                               signed __int64 *pRet) SAFEINT_CPP_THROW {
#if SAFEINT_USE_INTRINSICS
    ...
    LargeIntRegMultiply<unsigned __int64, unsigned __int32>::
        template RegMultiplyThrow<E>((unsigned __int64)a1, b, &tmp);
...
public:
  static bool RegMultiply(const signed __int64 &a, signed __int32 b,
                          signed __int64 *pRet) SAFEINT_NOTHROW {
#if SAFEINT_USE_INTRINSICS
    return IntrinsicMultiplyInt64(a, (signed __int64)b, pRet);
#else
  ...
  template <typename E>
  static void RegMultiplyThrow(signed __int64 a, signed __int32 b,
                               signed __int64 *pRet) SAFEINT_CPP_THROW {
#if SAFEINT_USE_INTRINSICS
    if (!IntrinsicMultiplyInt64(a, (signed __int64)b, pRet))
      E::SafeIntOnOverflow();
    ...
    if (a < 0) {
      aNegative = true;
      a = (signed __int64)AbsValueHelper<
          signed __int64, GetAbsMethod<signed __int64>::method>::Abs(a);
    }

    if (b < 0) {
      bNegative = true;
      b = (signed __int32)AbsValueHelper<
          signed __int32, GetAbsMethod<signed __int32>::method>::Abs(b);
    }

    LargeIntRegMultiply<unsigned __int64, unsigned __int32>::
        template RegMultiplyThrow<E>((unsigned __int64)a, (unsigned __int32)b,
                                     &tmp);
...
public:
  static bool RegMultiply(signed __int32 a, const signed __int64 &b,
                          signed __int32 *pRet) SAFEINT_NOTHROW {
#if SAFEINT_USE_INTRINSICS
    __int64 tmp;
  ...
  template <typename E>
  static void RegMultiplyThrow(signed __int32 a, const signed __int64 &b,
                               signed __int32 *pRet) SAFEINT_CPP_THROW {
#if SAFEINT_USE_INTRINSICS
    __int64 tmp;
    ...
    if (b < 0) {
      bNegative = true;
      b2 = (signed __int64)AbsValueHelper<
          signed __int64, GetAbsMethod<signed __int64>::method>::Abs(b2);
    }

    LargeIntRegMultiply<unsigned __int32, unsigned __int64>::
        template RegMultiplyThrow<E>((unsigned __int32)a, (unsigned __int64)b2,
                                     &tmp);
...
public:
  ...
  static bool RegMultiply(const signed __int64 &a, const unsigned __int64 &b,
                          signed __int64 *pRet) SAFEINT_NOTHROW {
    bool aNegative = false;
...
// called, we need to properly transition types. The methods need __int64,
// __int32, etc. but the variables being passed to us could be long long, long
// int, or long, depending on the compiler. Microsoft compiler knows that long
// long is the same type as __int64, but gcc doesn't
...
public:
  ...
  template <typename E>
  static void MultiplyThrow(const unsigned __int64 &t,
...
public:
  ...
  static void MultiplyThrow(const T &t, const U &u, T &ret) SAFEINT_CPP_THROW {
    ...
    LargeIntRegMultiply<unsigned __int64, unsigned __int32>::
        template RegMultiplyThrow<E>(
            t1, (unsigned __int32)u,
            reinterpret_cast<unsigned __int64 *>(&ret));
  }
};
...
public:
  ...
  static bool Multiply(const T &t, const U &u, T &ret) SAFEINT_NOTHROW {
    ...
    if (LargeIntRegMultiply<unsigned __int32, unsigned __int64>::RegMultiply(
            t, u1, &tmp) &&
        SafeCastHelper<T, unsigned __int32,
                       GetCastMethod<T, unsigned __int32>::method>::Cast(tmp,
                                                                         ret)) {
      return true;
    }
  ...
  static void MultiplyThrow(const T &t, const U &u, T &ret) SAFEINT_CPP_THROW {
    ...
    LargeIntRegMultiply<unsigned __int32,
                        unsigned __int64>::template RegMultiplyThrow<E>(t, u1,
                                                                        &tmp);
    SafeCastHelper<
        T, unsigned __int32,
        GetCastMethod<T, unsigned __int32>::method>::template CastThrow<E>(tmp,
                                                                           ret);
  }
};
...
public:
  ...
  static void MultiplyThrow(const T &t, const U &u, T &ret) SAFEINT_CPP_THROW {
    C_ASSERT(IntTraits<T>::isUint64);
    unsigned __int64 t1 = t;
    LargeIntRegMultiply<unsigned __int64, signed __int32>::
        template RegMultiplyThrow<E>(
            t1, (signed __int32)u, reinterpret_cast<unsigned __int64 *>(&ret));
  }
};
...
public:
  ...
  static void MultiplyThrow(const T &t, const U &u, T &ret) SAFEINT_CPP_THROW {
    ...
    unsigned __int64 t1 = t;
    __int64 u1 = u;
    LargeIntRegMultiply<unsigned __int64, __int64>::template RegMultiplyThrow<
        E>(t1, u1, reinterpret_cast<unsigned __int64 *>(&ret));
  }
};
...
public:
  ...
  static bool Multiply(const T &t, const U &u, T &ret) SAFEINT_NOTHROW {
    ...
    if (LargeIntRegMultiply<unsigned __int32, __int64>::RegMultiply(
            (unsigned __int32)t, u1, &tmp) &&
        SafeCastHelper<T, unsigned __int32,
                       GetCastMethod<T, unsigned __int32>::method>::Cast(tmp,
                                                                         ret)) {
      return true;
  ...
  static void MultiplyThrow(const T &t, const U &u, T &ret) SAFEINT_CPP_THROW {
    ...
    LargeIntRegMultiply<unsigned __int32, __int64>::template RegMultiplyThrow<
        E>((unsigned __int32)t, u1, &tmp);
    SafeCastHelper<
        T, unsigned __int32,
        GetCastMethod<T, unsigned __int32>::method>::template CastThrow<E>(tmp,
                                                                           ret);
...
public:
  ...
  static void MultiplyThrow(const T &t, const U &u, T &ret) SAFEINT_CPP_THROW {
    C_ASSERT(IntTraits<T>::isInt64);
    __int64 t1 = t;
    LargeIntRegMultiply<__int64, unsigned __int32>::template RegMultiplyThrow<
        E>(t1, (unsigned __int32)u, reinterpret_cast<__int64 *>(&ret));
  }
};
...
public:
  ...
  static bool Multiply(const T &t, const U &u, T &ret) SAFEINT_NOTHROW {
    ...
    __int64 t1 = t;
    __int64 u1 = u;
    return LargeIntRegMultiply<__int64, __int64>::RegMultiply(
  ...
  static void MultiplyThrow(const T &t, const U &u, T &ret) SAFEINT_CPP_THROW {
    ...
    __int64 t1 = t;
    __int64 u1 = u;
    LargeIntRegMultiply<__int64, __int64>::template RegMultiplyThrow<E>(
        t1, u1, reinterpret_cast<__int64 *>(&ret));
  }
};
...
public:
  ...
  static bool Multiply(const T &t, U u, T &ret) SAFEINT_NOTHROW {
    C_ASSERT(IntTraits<T>::isInt64);
    __int64 t1 = t;
    return LargeIntRegMultiply<__int64, __int32>::RegMultiply(
...
public:
  ...
  static bool Multiply(T t, const U &u, T &ret) SAFEINT_NOTHROW {
    ...
    if (LargeIntRegMultiply<__int32, unsigned __int64>::RegMultiply((__int32)t,
                                                                    u1, &tmp) &&
        SafeCastHelper<T, __int32, GetCastMethod<T, __int32>::method>::Cast(
            tmp, ret)) {
      return true;
    }
  ...
  template <typename E>
  static void MultiplyThrow(T t, const unsigned __int64 &u,
                            T &ret) SAFEINT_CPP_THROW {
    C_ASSERT(IntTraits<U>::isUint64);
    unsigned __int64 u1 = u;
    __int32 tmp;

    LargeIntRegMultiply<__int32, unsigned __int64>::template RegMultiplyThrow<
        E>((__int32)t, u1, &tmp);
    SafeCastHelper<T, __int32, GetCastMethod<T, __int32>::method>::
...
public:
  ...
  static bool Multiply(const T &t, const U &u, T &ret) SAFEINT_NOTHROW {
    ...
    __int64 t1 = t;
    unsigned __int64 u1 = u;
    return LargeIntRegMultiply<__int64, unsigned __int64>::RegMultiply(
        t1, u1, reinterpret_cast<__int64 *>(&ret));
  }

  template <typename E>
  static void MultiplyThrow(const __int64 &t, const unsigned __int64 &u,
                            T &ret) SAFEINT_CPP_THROW {
    C_ASSERT(IntTraits<T>::isInt64 && IntTraits<U>::isUint64);
    __int64 t1 = t;
    unsigned __int64 u1 = u;
    LargeIntRegMultiply<__int64, unsigned __int64>::template RegMultiplyThrow<
        E>(t1, u1, reinterpret_cast<__int64 *>(&ret));
  }
};
...
public:
  ...
  static bool Multiply(T t, const U &u, T &ret) SAFEINT_NOTHROW {
    ...
    if (LargeIntRegMultiply<__int32, __int64>::RegMultiply((__int32)t, u1,
                                                           &tmp) &&
        SafeCastHelper<T, __int32, GetCastMethod<T, __int32>::method>::Cast(
            tmp, ret)) {
      return true;
  ...
  static void MultiplyThrow(T t, const U &u, T &ret) SAFEINT_CPP_THROW {
    ...
    LargeIntRegMultiply<__int32, __int64>::template RegMultiplyThrow<E>(
        (__int32)t, u1, &tmp);
    SafeCastHelper<T, __int32, GetCastMethod<T, __int32>::method>::
        template CastThrow<E>(tmp, ret);
  }
...
public:
  static SafeIntError Divide(const T &t, const U &u,
                             T &result) SAFEINT_NOTHROW {
    if (u == 0)
      return SafeIntDivideByZero;
...
public:
  static SafeIntError Divide(const T &t, const unsigned __int64 &u,
                             T &result) SAFEINT_NOTHROW {
    C_ASSERT(IntTraits<U>::isUint64);
    ...
    if (u <= (unsigned __int64)IntTraits<T>::maxInt) {
      ...
      if (t == IntTraits<T>::minInt &&
          u == (unsigned __int64)IntTraits<T>::minInt) {
  ...
  template <typename E>
  static void DivideThrow(const T &t, const unsigned __int64 &u,
                          T &result) SAFEINT_CPP_THROW {
    C_ASSERT(IntTraits<U>::isUint64);
...
public:
  ...
  template <typename E>
  static void AdditionThrow(const T &lhs, const U &rhs,
                            T &result) SAFEINT_CPP_THROW {
    // 16-bit or less unsigned addition
    __int32 tmp = lhs + rhs;
...
public:
  static bool Addition(const T &lhs, const U &rhs, T &result) SAFEINT_NOTHROW {
    ...
    if (tmp <= (__int32)IntTraits<T>::maxInt &&
        tmp >= (__int32)IntTraits<T>::minInt) {
      result = (T)tmp;
      return true;
    }
...
public:
  static bool Addition(const T &lhs, const U &rhs, T &result) SAFEINT_NOTHROW {
    ...
    if (rhs < 0) {
      if (lhs >=
          ~(unsigned __int64)(rhs) + 1) // negation is safe, since rhs is 64-bit
      {
        result = (T)(lhs + rhs);
        return true;
      }
...
public:
  static bool Addition(const T &lhs, const U &rhs, T &result) SAFEINT_NOTHROW {
    ...
    if (AdditionHelper<__int64, __int64, AdditionState_CastInt64CheckOverflow>::
            Addition((__int64)lhs, (__int64)rhs, tmp) &&
  ...
  static void AdditionThrow(const T &lhs, const U &rhs,
    ...
    AdditionHelper<__int64, __int64, AdditionState_CastInt64CheckOverflow>::
        AdditionThrow<E>((__int64)lhs, (__int64)rhs, tmp);
...
public:
  static bool Addition(const __int64 &lhs, const unsigned __int64 &rhs,
                       __int64 &result) SAFEINT_NOTHROW {
    C_ASSERT(IntTraits<T>::isInt64 && IntTraits<U>::isUint64);
  ...
  template <typename E>
  static void AdditionThrow(const __int64 &lhs, const unsigned __int64 &rhs,
                            T &result) SAFEINT_CPP_THROW {
    C_ASSERT(IntTraits<T>::isInt64 && IntTraits<U>::isUint64);
    // rhs is unsigned __int64, lhs __int64
    unsigned __int64 tmp = (unsigned __int64)lhs + rhs;
...
public:
  ...
  static void AdditionThrow(const T &lhs, const U &rhs,
    ...
    if ((unsigned __int32)(rhs >> 32) == 0) {
      ...
      if ((__int32)tmp >= lhs) {
        SafeCastHelper<T, __int32, GetCastMethod<T, __int32>::method>::
            template CastThrow<E>((__int32)tmp, result);
        return;
      }
    }
    E::SafeIntOnOverflow();
...
public:
  ...
  template <typename E>
  static void SubtractThrow(const T &lhs, const U &rhs,
                            T &result) SAFEINT_CPP_THROW {
    // both are unsigned - easy case
    if (rhs <= lhs) {
      result = (T)(lhs - rhs);
...
public:
  static bool Subtract(const T &lhs, const U &rhs, T &result) SAFEINT_NOTHROW {
    ...
    if (SafeCastHelper<T, __int32, GetCastMethod<T, __int32>::method>::Cast(
            tmp, result)) {
      result = (T)tmp;
      return true;
    }
...
public:
  static bool Subtract(const U &lhs, const T &rhs, T &result) SAFEINT_NOTHROW {
    ...
    return SafeCastHelper<T, __int32, GetCastMethod<T, __int32>::method>::Cast(
        tmp, result);
  }

  template <typename E>
  static void SubtractThrow(const U &lhs, const T &rhs,
                            T &result) SAFEINT_CPP_THROW {
    // both values are 16-bit or less
    // rhs is signed, so could end up increasing or decreasing
    __int32 tmp = lhs - rhs;
...
public:
  static bool Subtract(const T &lhs, const U &rhs, T &result) SAFEINT_NOTHROW {
    ...
    return SafeCastHelper<T, __int64, GetCastMethod<T, __int64>::method>::Cast(
        tmp, result);
  }
  ...
  static void SubtractThrow(const T &lhs, const U &rhs,
    ...
    SafeCastHelper<T, __int64, GetCastMethod<T, __int64>::method>::
        template CastThrow<E>(tmp, result);
  }
};
...
public:
  static bool Subtract(const U &lhs, const T &rhs, T &result) SAFEINT_NOTHROW {
    ...
    if (rhs < 0) {
      ...
      tmp = lhs +
            (unsigned __int64)AbsValueHelper<T, GetAbsMethod<T>::method>::Abs(
                rhs);
...
public:
  static bool Subtract(const T &lhs, const U &rhs, T &result) SAFEINT_NOTHROW {
    ...
    if (rhs >= 0) {
      ...
      unsigned __int64 tmp =
          lhs + ~(unsigned __int64)(rhs) + 1; // negation safe
...
public:
  ...
  static void SubtractThrow(const U &lhs, const T &rhs,
    ...
    if (lhs >= 0) {
      // if both positive, overflow to negative not possible
      // which is why we'll explicitly check maxInt, and not call SafeCast
      if ((CompileConst<IntTraits<T>::isLT64Bit>::Value() &&
...
public:
  ...
  static bool Subtract(const U &lhs, const T &rhs, T &result) SAFEINT_NOTHROW {
    ...
    if ((__int64)tmp <= IntTraits<T>::maxInt &&
        (__int64)tmp >= IntTraits<T>::minInt) {
      result = (T)(__int64)tmp;
      return true;
    }
...
public:
  static bool Subtract(const T &lhs, const U &rhs, T &result) SAFEINT_NOTHROW {
    ...
    const unsigned __int64 AbsMinIntT =
        (unsigned __int64)IntTraits<T>::maxInt + 1;
...
public:
  static bool Subtract(const __int64 &lhs, const unsigned __int64 &rhs,
                       __int64 &result) SAFEINT_NOTHROW {
    C_ASSERT(IntTraits<T>::isInt64 && IntTraits<U>::isUint64);
  ...
  template <typename E>
  static void SubtractThrow(const __int64 &lhs, const unsigned __int64 &rhs,
                            T &result) SAFEINT_CPP_THROW {
    C_ASSERT(IntTraits<T>::isInt64 && IntTraits<U>::isUint64);
    // if we subtract, and it gets larger, there's a problem
    // Perform test as unsigned to prevent unwanted optimizations
...
public:
  enum {
    ...
    method = (sizeof(T) <= sizeof(U) || SafeIntCompare<T, U>::isBothUnsigned ||
              !IntTraits<U>::isSigned)
                 ? BinaryState_OK
...
public:
  ...
  template <typename U> SafeInt(const SafeInt<U, E> &u) SAFEINT_CPP_THROW {
    C_ASSERT(NumericType<T>::isInt);
    *this = SafeInt<T, E>((U)u);
  }

  template <typename U> SafeInt(const U &i) SAFEINT_CPP_THROW {
    C_ASSERT(NumericType<T>::isInt);
  ...
  operator __int16() const SAFEINT_CPP_THROW {
    __int16 val;
    SafeCastHelper<__int16, T, GetCastMethod<__int16, T>::method>::
        template CastThrow<E>(m_int, val);
    return val;
  }
  ...
  operator unsigned __int16() const SAFEINT_CPP_THROW {
    ...
    SafeCastHelper<unsigned __int16, T,
                   GetCastMethod<unsigned __int16,
  ...
  operator __int32() const SAFEINT_CPP_THROW {
    __int32 val;
    SafeCastHelper<__int32, T, GetCastMethod<__int32, T>::method>::
        template CastThrow<E>(m_int, val);
    return val;
  }
  ...
  operator unsigned __int32() const SAFEINT_CPP_THROW {
    ...
    SafeCastHelper<unsigned __int32, T,
                   GetCastMethod<unsigned __int32,
                                 T>::method>::template CastThrow<E>(m_int, val);
    return val;
  }
  ...
  operator __int64() const SAFEINT_CPP_THROW {
    __int64 val;
    SafeCastHelper<__int64, T, GetCastMethod<__int64, T>::method>::
        template CastThrow<E>(m_int, val);
    return val;
  }
  ...
  operator unsigned __int64() const SAFEINT_CPP_THROW {
    ...
    SafeCastHelper<unsigned __int64, T,
                   GetCastMethod<unsigned __int64,
                                 T>::method>::template CastThrow<E>(m_int, val);
    return val;
  }
  ...
  SafeInt<T, E> operator-() const SAFEINT_CPP_THROW {
    ...
    return SafeInt<T, E>(
        NegationHelper<T, IntTraits<T>::isSigned>::template NegativeThrow<E>(
            m_int));
  }
  ...
  template <typename U> SafeInt<T, E> operator%(U rhs) const SAFEINT_CPP_THROW {
    T result;
  ...
  // Modulus assignment
  template <typename U> SafeInt<T, E> &operator%=(U rhs) SAFEINT_CPP_THROW {
    ModulusHelper<T, U, ValidComparison<T, U>::method>::template ModulusThrow<
        E>(m_int, rhs, m_int);
    return *this;
  }
...
#define ShiftAssert(x) SAFEINT_ASSERT(x)
  ...
  template <typename U> SafeInt<T, E> operator<<(U bits) const SAFEINT_NOTHROW {
  ...
  template <typename U> SafeInt<T, E> &operator<<=(U bits) SAFEINT_NOTHROW {
    ShiftAssert(!IntTraits<U>::isSigned || bits >= 0);
    ShiftAssert(bits < (int)IntTraits<T>::bitCount);
  ...
  template <typename U> SafeInt<T, E> operator&(U rhs) const SAFEINT_NOTHROW {
  ...
  template <typename U> SafeInt<T, E> &operator&=(U rhs) SAFEINT_NOTHROW {
  ...
  Min(SafeInt<T, E> test,
      const T floor = IntTraits<T>::minInt) const SAFEINT_NOTHROW {
  ...
  Max(SafeInt<T, E> test,
      const T upper = IntTraits<T>::maxInt) const SAFEINT_NOTHROW {
    T tmp = test > m_int ? (T)test : m_int;
  ...
  template <alignBits bits> const SafeInt<T, E> &Align() SAFEINT_CPP_THROW {
    ...
    ShiftAssert(
        ((IntTraits<T>::isSigned && bits < (int)IntTraits<T>::bitCount - 1) ||
...
template <typename P>
SafeInt<ptrdiff_t, SafeIntDefaultExceptionHandler>
...
public:
  static bool SignedCase(SafeInt<T, E> rhs,
...
public:
  static bool ModulusSimpleCase(U lhs, SafeInt<T, E> rhs,
                                SafeInt<T, E> &result) SAFEINT_CPP_THROW {
    if (rhs != 0) {
      if (ModulusSignedCaseHelper<T, E, IntTraits<T>::isSigned>::SignedCase(
              rhs, result))
        return true;
...
SafeInt<T, E> operator%(U lhs, SafeInt<T, E> rhs) SAFEINT_CPP_THROW {
  ...
  if (ModulusSimpleCaseHelper < T, U, E,
      sizeof(T) == sizeof(U) && (bool)IntTraits<T>::isSigned ==
                                    (bool)IntTraits<U>::isSigned >
...
public:
  static bool NegativeCornerCase(U lhs, SafeInt<T, E> rhs,
                                 SafeInt<T, E> &result) SAFEINT_CPP_THROW {
...
public:
  static bool DivisionCornerCase1(U lhs, SafeInt<T, E> rhs,
                                  SafeInt<T, E> &result) SAFEINT_CPP_THROW {
    if ((T)rhs > 0) {
...
public:
  static bool DivisionCornerCase2(U lhs, SafeInt<T, E> rhs,
                                  SafeInt<T, E> &result) SAFEINT_CPP_THROW {
...
SafeInt<T, E> operator/(U lhs, SafeInt<T, E> rhs) SAFEINT_CPP_THROW {
  ...
  if (DivisionCornerCaseHelper2<T, U, E, SafeIntCompare<T, U>::isBothSigned>::
