...
static std::string build_error_msg(const std::error_code &ec,
                                   const std::string &location) {
  std::stringstream ss;
  ss.imbue(std::locale::classic());
  ss << location << ": " << ec.value() << ": " << ec.message();
  return ss.str();
}

static utility::string_t
    g_subProtocolHeader(_XPLATSTR("Sec-WebSocket-Protocol"));

class wspp_callback_client
    : public websocket_client_callback_impl,
      public std::enable_shared_from_this<wspp_callback_client> {
private:
  enum State { CREATED, CONNECTING, CONNECTED, CLOSING, CLOSED, DESTROYED };

public:
  wspp_callback_client(websocket_client_config config)
      : websocket_client_callback_impl(std::move(config)), m_state(CREATED),
        m_num_sends(0)
  ...
  pplx::task<void> connect() {
    if (m_uri.scheme() == U("wss")) {
      m_client = std::unique_ptr<websocketpp_client_base>(
          new websocketpp_tls_client());
    ...
    defined(_WIN32)
        m_openssl_failed = false;
#endif
        sslContext->set_verify_callback([this](bool preverified,
                                               boost::asio::ssl::verify_context
                                                   &verifyCtx) {
#if defined(__APPLE__) || (defined(ANDROID) || defined(__ANDROID__)) ||        \
    defined(_WIN32)
          // On OS X, iOS, and Android, OpenSSL doesn't have access to where the
          // OS stores keychains. If OpenSSL fails we will doing verification at
          // the end using the whole certificate chain so wait until the 'leaf'
          // cert. For now return true so OpenSSL continues down the certificate
          // chain.
          if (!preverified) {
        ...
        // OpenSSL stores some per thread state that never will be cleaned up
        // until the dll is unloaded. If static linking, like we do, the state
        // isn't cleaned up at all and will be reported as leaks. See
        // http://www.openssl.org/support/faq.html#PROG13 This is necessary here
        // because it is called on the user's thread calling connect(...)
      ...
      client.set_socket_init_handler(
          [this](websocketpp::connection_hdl,
            ...
            if (m_config.is_sni_enabled()) {
              // If user specified server name is empty default to use URI host
              // name.
              if (!m_config.server_name().empty()) {
                // OpenSSL runs the string parameter through a macro casting
                // away const with a C style cast. Do a C++ cast ourselves to
                // avoid warnings.
      ...
      m_client =
          std::unique_ptr<websocketpp_client_base>(new websocketpp_client());
      return connect_impl<websocketpp::config::asio_client>();
    }
  }

  template <typename WebsocketConfigType> pplx::task<void> connect_impl() {
        ...
        [this](websocketpp::connection_hdl,
          ...
          if (m_external_message_handler) {
            ...
            case websocketpp::frame::opcode::text:
              ...
              break;
            default:
              // Unknown message type. Since both websocketpp and our code use
              // the RFC codes, we'll just pass it on to the user.
              incoming_msg.m_msg_type =
                  static_cast<websocket_message_type>(msg->get_opcode());
              break;
            }
            ...
            incoming_msg.m_body =
                concurrency::streams::container_buffer<std::string>(
                    std::move(payload));
    ...
    auto con = client.get_connection(
        utility::conversions::to_utf8string(m_uri.to_string()), ec);
    m_con = con;
    if (ec.value() != 0) {
      return pplx::task_from_exception<void>(
          websocket_exception(ec, build_error_msg(ec, "get_connection")));
    }
    ...
    if (proxy.is_specified()) {
      con->set_proxy(
          utility::conversions::to_utf8string(proxy.address().to_string()), ec);
      if (ec) {
        return pplx::task_from_exception<void>(
            websocket_exception(ec, build_error_msg(ec, "set_proxy")));
      }
      ...
      if (cred.is_set()) {
        con->set_proxy_basic_auth(
            ...
            utility::conversions::to_utf8string(*cred.decrypt()), ec);
        if (ec) {
          return pplx::task_from_exception<void>(websocket_exception(
              ec, build_error_msg(ec, "set_proxy_basic_auth")));
        }
      }
    }
  ...
  pplx::task<void> send(websocket_outgoing_message &msg) {
    if (!m_connect_tce._IsTriggered()) {
      return pplx::task_from_exception<void>(
          websocket_exception("Client not connected."));
    }
    ...
    default:
      return pplx::task_from_exception<void>(
          websocket_exception("Invalid message type"));
    }

    const auto length = msg.m_length;
    if (length == 0 && msg.m_msg_type != websocket_message_type::pong) {
      return pplx::task_from_exception<void>(
          websocket_exception("Cannot send empty message."));
    }
    if (length >= UINT_MAX && length != SIZE_MAX) {
      return pplx::task_from_exception<void>(
          websocket_exception("Message size too large. Ensure message length "
  ...
  void send_msg(websocket_outgoing_message &msg) {
    ...
    if (length == SIZE_MAX) {
      ...
      if (is_buf.has_size()) {
        ...
        if (buf_sz >= SIZE_MAX) {
          msg.signal_body_sent(std::make_exception_ptr(websocket_exception(
              "Cannot send messages larger than SIZE_MAX.")));
          return;
        }
        ...
        msg.m_body =
            concurrency::streams::container_buffer<std::vector<uint8_t>>();
        is_buf_istream.read_to_end(msg.m_body)
            .then([this_client, msg](pplx::task<size_t> t) mutable {
              try {
                msg.m_length = t.get();
        ...
        // We have postponed the call to send_msg() until after the data is
        // buffered.
        return;
      }
    }

    // First try to acquire the data (Get a pointer to the next already
    // allocated contiguous block of data) If acquire succeeds, send the data
    // over the socket connection, there is no copy of data from stream to
    // temporary buffer. If acquire fails, copy the data to a temporary buffer
    // managed by sp_allocated and send it over the socket connection.
    ...
    if (!acquired ||
        acquired_size < length) // Stream does not support acquire or failed to
                                // acquire specified number of bytes
    {
      // If acquire did not return the required number of bytes, do not rely on
      // its return value.
      ...
      read_task =
          is_buf.getn(sp_allocated.get(), length)
              .then([length](size_t bytes_read) {
                if (bytes_read != length) {
                  throw websocket_exception("Failed to read required length of "
                                            "data from the stream.");
                }
              });
    } else {
      // Acquire succeeded, assign the acquired pointer to sp_allocated. Use an
      // empty custom destructor so that the data is not released when
      // sp_allocated goes out of scope. The streambuf will manage its memory.
    ...
    read_task
        ...
        .then(
            [this_client, msg, is_buf, acquired, sp_allocated, length](
              ...
              try {
                // Catch exceptions from previous tasks, if any and convert it
                // to websocket exception.
                const auto &ec = previousTask.get();
                if (ec.value() != 0) {
                  eptr = std::make_exception_ptr(websocket_exception(
                      ec, build_error_msg(ec, "sending message")));
                }
              } catch (...) {
  ...
  pplx::task<void> close() {
    return close(
        static_cast<websocket_close_status>(websocketpp::close::status::normal),
        U("Normal"));
  }

  pplx::task<void> close(websocket_close_status status,
...
private:
  ...
  void shutdown_wspp_impl(const websocketpp::connection_hdl &con_hdl,
                          bool connecting) {
    ...
    pplx::create_task([this, connecting, ec, closeCode, reason] {
      ...
      // Delete client to make sure Websocketpp cleans up all Boost.Asio
      // portions.
      m_client.reset();
      ...
      if (m_external_close_handler) {
        m_external_close_handler(static_cast<websocket_close_status>(closeCode),
                                 utility::conversions::to_string_t(reason), ec);
      }
      // Making a local copy of the TCE prevents it from being destroyed along
      // with "this"
  ...
  static void
  send_msg_impl(const std::shared_ptr<wspp_callback_client> &this_client,
                const websocket_outgoing_message &msg,
                const std::shared_ptr<uint8_t> &sp_allocated, size_t length,
                websocketpp::lib::error_code &ec) {
    auto &client = this_client->m_client->client<WebsocketClientType>();
    switch (msg.m_msg_type) {
    case websocket_message_type::text_message:
      client.send(this_client->m_con, sp_allocated.get(), length,
    ...
    case websocket_message_type::binary_message:
      ...
      break;
    case websocket_message_type::pong:
      client.pong(this_client->m_con, "", ec);
      break;
    default:
  ...
  void close_impl(websocket_close_status status,
                  const utility::string_t &reason,
                  websocketpp::lib::error_code &ec) {
    auto &client = m_client->client<WebsocketConfig>();
    client.close(m_con, static_cast<websocketpp::close::status::value>(status),
                 utility::conversions::to_utf8string(reason), ec);
  }

  void set_message_handler(
      const std::function<void(const websocket_incoming_message &)> &handler) {
    m_external_message_handler = handler;
  }

  void set_close_handler(const std::function<
                         void(websocket_close_status, const utility::string_t &,
  ...
  struct websocketpp_client_base {
    ...
    websocketpp::client<WebsocketConfig> &client() {
      if (is_tls_client()) {
        return reinterpret_cast<websocketpp::client<WebsocketConfig> &>(
            tls_client());
      } else {
        return reinterpret_cast<websocketpp::client<WebsocketConfig> &>(
            non_tls_client());
    ...
    virtual websocketpp::client<websocketpp::config::asio_client> &
    non_tls_client() {
      throw std::bad_cast();
    }
    virtual websocketpp::client<websocketpp::config::asio_tls_client> &
    tls_client() {
      throw std::bad_cast();
    }
...
websocket_client_task_impl::websocket_client_task_impl(
    ...
    : m_client_closed(false),
      m_callback_client(
          std::make_shared<details::wspp_callback_client>(std::move(config))) {
  set_handler();
}
} // namespace details

websocket_callback_client::websocket_callback_client()
    : m_client(std::make_shared<details::wspp_callback_client>(
